...
async function seedOldCategories() {
  try {
    console.log('🌱 Starting old categories seeding with full information...');
    ...
    for (const categoryData of oldCategories) {
      ...
      console.log(`✅ Created category: ${categoryData.name.en} (${categoryData.slug})`);
    }

    console.log(`🎉 Successfully seeded ${createdCategories.length} categories!`);
    console.log('\n📋 Created categories:');
    createdCategories.forEach(category => {
      console.log(`   - ${category.name.en} (${category.slug}) - Order: ${category.order}`);
    });

    return createdCategories;

  } catch (error) {
    console.error('❌ Error seeding old categories:', error);
    throw error;
  } finally {
...
if (require.main === module) {
  seedOldCategories()
    .then(() => {
      console.log('✅ Old categories seeding completed successfully!');
      process.exit(0);
    })
    .catch((error) => {
      console.error('❌ Old categories seeding failed:', error);
      process.exit(1);
    });
}

module.exports = seedOldCategories;
